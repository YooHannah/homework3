...
module.exports = function (inst) {
  if (storage.compat()) {
    storage.init()
    inst.plugin('options', function (options) {  //判断数据请求方式
    ...
    inst.plugin('endpoint', function (options, defer) { //根据options处理结果获取数据

      if (options.hasCache) return fromCache.bind(inst, defer) //从catch中返回请求的数据
      // mark the start time
      options.__start__ = +new Date //计算请求需要时长
     //在inst的继承类中写endpoint的plugin，依靠tapable机制，会接着执行继承类中的endpoint
     //因此在继承类的endpoint中就可以进行不同接口请求操作
    })
    inst.plugin('resolve', function (res, opts, defer) { //接口数据获取完，对缓存进行处理操作
      // use cache
