 ...
 */
const PROTOCOL_VERSION = 1  //当后台给过来的数据结构要发生变化,就手动修改,从而在localstorage中找不到，进而请求存储成新的数据结构，v也跟着变成新修改的PROTOCOL_VERSION
const _ = {}//装数据当副本
let keys = []
...
function get(key) {
  ...
  if (!res) {
    ...
    } catch(e) {//读取localstorage有问题，很有可能整个localstorage有问题，就清空整个localStorage
      keys.length = 0
